module VMTests.VmOperatorTest (vmTestOperator) where
import Test.HUnit
import VirtualMachine.VMConstants
import VirtualMachine.StackMachine

vmTestOperator :: Test
vmTestOperator = TestList [additionTest1, additionTest2, additionTest3, additionTest4, additionTest5]
...
additionTest1 = TestCase $ do
  ...
  where
    program = [Push (IntVal 2), Push (IntVal 3), Push (OpVal Add1), Ret]
...
additionTest2 = TestCase $ do
  ...
  where
    program = [Push (IntVal (-5)), Push (IntVal 8), Push (OpVal Add1), Ret]
...
additionTest3 = TestCase $ do
  ...
  where
    program = [Push (IntVal 0), Push (IntVal 7), Push (OpVal Add1), Ret]
...
additionTest4 = TestCase $ do
  ...
  where
    program = [Push (IntVal 1000), Push (IntVal 2000), Push (OpVal Add1), Ret]
...
additionTest5 = TestCase $ do
  ...
  where
    program = [Push (IntVal (-10)), Push (IntVal 15), Push (OpVal Add1), Ret]
    expected = Right (IntVal 5)